...
matrix::Vector3f smc::SMC_Control(Vector3f att_sp , Vector3f att , Vector3f rate ,float dt)
{
	...
	Vector3f torque;
	torque(0)= smc::Ixx*(ddatt_sp(0)+datt_sp(0)-rate(0)-f_roll-smc::x*sat(s(0))-smc::k*s(0));
	torque(1)= smc::Iyy*(ddatt_sp(1)+datt_sp(1)-rate(1)-f_pitch-smc::x*sat(s(1))-smc::k*s(1));
	torque(2)= smc::Izz*(ddatt_sp(2)+datt_sp(2)-rate(2)-f_yaw-smc::x*sat(s(2))-smc::k*s(2));
